...
class MultiSpeakerRnntModel(nn.Module):
    ...
    def forward_train_routing(
        ...
        return_rnnt_simple_logits: bool = False,
    ) -> Tuple[Tensor, ...]:
        ...
        use_transducer = getattr(self.asr_model, "use_transducer", True)
        if use_transducer:
            if return_rnnt_simple_logits:
                (
                    simple_loss,
                    pruned_loss,
                    student_rnnt_am_logits,
                    student_rnnt_lm_logits,
                    student_decoder_input_ids,
                    student_target_lens,
                    student_encoder_out,
                    student_decoder_out,
                ) = self.asr_model.forward_transducer(
                    encoder_out=H_loss,
        ...
        if getattr(self.asr_model, "use_ctc", False) and hasattr(
            ...
            if return_ctc_logits:
                ctc_logits = self.asr_model.get_ctc_logits(H_loss)  # [N, T, V]
        else:
            ctc_loss = torch.tensor(0.0, device=device)

        if return_ctc_logits or return_rnnt_simple_logits:
            kd_outputs = {
                "student_enc_lens": enc_lens_loss,
            }
            if return_ctc_logits:
                kd_outputs["student_ctc_logits"] = ctc_logits
            if return_rnnt_simple_logits:
                kd_outputs["student_rnnt_am_logits"] = student_rnnt_am_logits
                kd_outputs["student_rnnt_lm_logits"] = student_rnnt_lm_logits
                kd_outputs["student_decoder_input_ids"] = student_decoder_input_ids
                kd_outputs["student_target_lens"] = student_target_lens
                kd_outputs["student_encoder_out"] = student_encoder_out
                kd_outputs["student_decoder_out"] = student_decoder_out

            return simple_loss, pruned_loss, ctc_loss, kd_outputs

        return simple_loss, pruned_loss, ctc_loss

    def forward_kd_features(
        self,
        source: Tensor,
        padding_mask: Tensor,
    ) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        """Run student HuBERT+Zipformer2 on full audio WITHOUT SpeakerKernel.

        Extracts per-stack intermediate features for FitNet/MGD distillation.
        Because the teacher also processes full (un-routed) audio, bypassing
        SpeakerKernel here gives content-matched features for KD.

        Gradients flow through the returned features, allowing both the
        projection heads (kd_fitnet_proj3 / kd_mgd_proj5) and the student
        encoder to receive KD gradient signal.

        Args:
            source:       raw waveform  [B, T_samples]
            padding_mask: True = padded [B, T_samples]

        Returns:
            stack3_feat: [B, T_out, 512]  encoders[3] output (no SpeakerKernel)
            stack5_feat: [B, T_out, 256]  encoders[5] output
            enc_out:     [B, T_out, D]    full encoder output
            enc_lens:    [B]              valid frame counts
        """
        hubert = self.asr_model.encoder
        zipformer = hubert.encoder
        ...
        if padding_mask is None:
            padding_mask = torch.zeros(
                B, source.shape[1], dtype=torch.bool, device=device
            )

        valid_samples = (~padding_mask).sum(dim=1).to(dtype=torch.long, device=device)

        # Step 1: HuBERT conv frontend on full audio (same path as routing pass)
        raw_features = hubert.feature_extractor(source)   # [B, C, T_feat]
        T_feat = int(raw_features.shape[2])
        x_lens_batch = self._samples_to_feature_lengths(
            valid_samples, hubert.feature_extractor
        ...
        src_key_padding_mask = (
            torch.arange(T_feat, device=device).unsqueeze(0)
            >= x_lens_batch.unsqueeze(1)
        )  # [B, T_feat]

        # Step 2: Zipformer2 forward WITHOUT SpeakerKernel block_callbacks
        old_callbacks = getattr(zipformer, "block_callbacks", {})
        zipformer.block_callbacks = {}

        stack3_feat: List[Optional[Tensor]] = [None]
        stack5_feat: List[Optional[Tensor]] = [None]

        def _hook3(module, inp, output):
            # output: [T_out, B, encoder_dim[3]]  →  [B, T_out, encoder_dim[3]]
            stack3_feat[0] = output.permute(1, 0, 2)

        def _hook5(module, inp, output):
            stack5_feat[0] = output.permute(1, 0, 2)

        h3 = zipformer.encoders[3].register_forward_hook(_hook3)
        h5 = zipformer.encoders[5].register_forward_hook(_hook5)

        try:
            x = features.transpose(0, 1)                  # [T_feat, B, D]
            enc_out_tbd, enc_lens = zipformer(
                x, x_lens_batch, src_key_padding_mask=src_key_padding_mask
            )
            enc_out = enc_out_tbd.transpose(0, 1)          # [B, T_out, D]
        finally:
            h3.remove()
            h5.remove()
            zipformer.block_callbacks = old_callbacks

        assert stack3_feat[0] is not None, "Hook on student encoders[3] did not fire"
        assert stack5_feat[0] is not None, "Hook on student encoders[5] did not fire"

        return stack3_feat[0], stack5_feat[0], enc_out, enc_lens
    ...
    def _chunked_forward_pre_asr_stream_routing(
        ...
        if getattr(self.asr_model, "use_transducer", True):
            simple_loss, pruned_loss = self.asr_model.forward_transducer(
                encoder_out=H_all,
