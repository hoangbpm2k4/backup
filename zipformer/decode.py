...
def extract_ref_texts(batch: dict) -> List[str]:
    """Extract per-cut reference texts from either legacy or multi-speaker batch."""
    sups = batch.get("supervisions")
    if isinstance(sups, dict) and "text" in sups:
        return list(sups["text"])

    y_texts = batch.get("y_texts")
    if y_texts is None or len(y_texts) == 0:
        return ["" for _ in batch.get("cuts", [])]

    batch_size = len(y_texts[0])
    texts: List[str] = []
    for b in range(batch_size):
        picked = ""
        for s in range(len(y_texts)):
            if b < len(y_texts[s]) and y_texts[s][b]:
                picked = y_texts[s][b]
                break
        texts.append(picked)
    return texts
...
def decode_one_batch(
    ...
    decode_model = model.asr_model if hasattr(model, "asr_model") else model
    encoder_out, encoder_out_lens = decode_model.forward_encoder(audio, padding_mask)

    hyps = []

    if params.decoding_method == "ctc_greedy_search":
        if not hasattr(decode_model, "ctc_output"):
            raise AttributeError(
                "CTC decoding requested but model has no ctc_output head."
            )

        ctc_logits = decode_model.ctc_output(encoder_out)  # [B, T, V]
        best_ids = ctc_logits.argmax(dim=-1)  # [B, T]
        blank_id = int(params.blank_id)

        for i in range(best_ids.size(0)):
            t = int(encoder_out_lens[i].item())
            prev = -1
            tokens = []
            for tok in best_ids[i, :t].tolist():
        ...
        ref_texts = extract_ref_texts(batch)
...
def decode_dataset(
    ...
    if params.decoding_method in ("greedy_search", "ctc_greedy_search"):
        log_interval = 50
    else:
        log_interval = 20
...
def main():
    ...
    if str(params.bpe_model).endswith(".txt"):
        logging.info(f"Using TokenEncoder with tokens: {params.bpe_model}")
    ...
    active_eval_sets = []
    for test_set, cuts in eval_cutsets:
        n_cuts = len(cuts)
        if n_cuts == 0:
            logging.info(f"Skip {test_set}: no cuts found")
            continue
        test_dl = librispeech.test_dataloaders(
            cuts,
            do_normalize=params.do_normalize,
        )
        active_eval_sets.append((test_set, test_dl))

    for test_set, test_dl in active_eval_sets:
