...
class Zipformer2(EncoderInterface):
    ...
    def get_chunk_info(self) -> Tuple[int, int]:
        ...
        When chunk_size and left_context_frames have the same length,
        they are treated as paired configs (index i maps to index i).
        A single pair is chosen per call with inverse-chunk-size weighting
        so that smaller chunks are sampled more often (they contribute
        less gradient per step and need more exposure).
        """
        if not self.causal:
            return -1, -1
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            ...
            if len(self.chunk_size) == len(self.left_context_frames):
                # Paired configs: pick one index with inverse-size weighting
                weights = [1.0 / max(cs, 1) for cs in self.chunk_size]
                total = sum(weights)
                weights = [w / total for w in weights]
                idx = random.choices(range(len(self.chunk_size)), weights=weights, k=1)[0]
                chunk_size = self.chunk_size[idx]
                left_context_frames = self.left_context_frames[idx]
            else:
                # Fallback: independent random (backward compat)
    ...
    def streaming_forward(
        ...
        Args:
          ...
          src_key_padding_mask:
            ...
            masked position. May be None.
          left_context_frames:
            Optional runtime override for base-rate left context size.
            Use -1 to keep the configured default (self.left_context_frames[0]).
        Returns:
          Return a tuple containing 2 tensors:
            - embeddings: its shape is (output_seq_len, batch_size, max(encoder_dim))
            - lengths, a tensor of shape (batch_size,) containing the number
        ...
        for i, module in enumerate(self.encoders):
            ...
            x, new_layer_states = module.streaming_forward(
                ...
                left_context_len=left_context_frames // ds,
        ...
        if self.output_downsampling_factor != 1:
            ...
            # output_downsampling_factor == 1: no downsampling, consistent
            # with forward() which also skips downsample_output.
            lengths = x_lens
    ...
    def get_init_states(
        ...
        for i, module in enumerate(self.encoders):
            ...
            downsample_left = left_context_frames // ds
...
class Zipformer2Encoder(nn.Module):
    ...
    def streaming_forward(
        ...
        for i, mod in enumerate(self.layers):
            ...
            if use_gc:
                def _layer_fn(_mod, _output, _pos_emb,
                              _ck, _cna, _cv1, _cv2, _cc1, _cc2,
                              _lcl, _skpm):
                    return _mod.streaming_forward(
                        _output, _pos_emb,
                        cached_key=_ck, cached_nonlin_attn=_cna,
                        cached_val1=_cv1, cached_val2=_cv2,
                        cached_conv1=_cc1, cached_conv2=_cc2,
                        left_context_len=_lcl,
                        src_key_padding_mask=_skpm,
                    )
                (
                    output,
                    new_cached_key,
                    ...
                    _layer_fn,
                    mod, output, pos_emb,
                    cached_key, cached_nonlin_attn,
                    cached_val1, cached_val2,
                    cached_conv1, cached_conv2,
                    left_context_len, src_key_padding_mask,
                    use_reentrant=True,
